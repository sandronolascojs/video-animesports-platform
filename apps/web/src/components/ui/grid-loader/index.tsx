...
export interface GridLoaderProps {
  ...
  className?: string;
  /** Color preset, custom CSS color, or an array of them — an array paints the
   *  cells multicolor, each cell cycling the next color by its grid position. */
  color?: "white" | "red" | "blue" | "green" | "amber" | string | string[];
...
const GridLoader = ({
  ...
  // A single color paints every cell the same; an array paints them multicolor,
  // each cell picking `colors[idx % colors.length]` by its grid position.
  const resolvedColors = Array.isArray(color)
    ? color.map((entry) => resolveColor(entry))
    : [resolveColor(color)];
    ...
    >
      {cells.map((active, idx) => (
        <GridCell
          ...
          cellSize={cellSize}
          color={resolvedColors[idx % resolvedColors.length]}
